...
int ReadPageTable(const char *filename, PageTable *pt)
{
	...
	fscanf(fp, "%s %d", buffer, &pt->bits_for_page_number);
	fscanf(fp, "%s %d", buffer, &pt->bits_for_offset);
	fscanf(fp, "%s %d", buffer, &pt->page_table_length);

	if(pt->bits_for_offset > 0)
		pt->page_size = 1 << pt->bits_for_offset;
	pt->limit = pt->page_size*pt->page_table_length;

	pt->frame_number = (int*)malloc(sizeof(int)*(pt->page_size));
...
void DestroyPageTable(PageTable *pt)
{
	pt->bits_for_page_number = pt->bits_for_offset = pt->limit = pt->page_size = pt->page_table_length = 0 ;
...
int Translate(int logical_addr, PageTable *pt)
{
	...
	*/
	int frameNumber,offset, result;
	frameNumber = offset = result =0;

	frameNumber = pt->frame_number[logical_addr>>pt->bits_for_offset];
	offset = logical_addr % pt->page_size;
	result = (frameNumber << pt->bits_for_offset) | offset;
